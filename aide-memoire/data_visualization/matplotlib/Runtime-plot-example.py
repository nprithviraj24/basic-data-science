...
def animate(i):
    ...
    pullData = open('calculated-accuracy.txt', 'r').read()        
    ...
    plt.xticks(np.arange(x.min(), counter, 1))
    ...
    for eachLine in dataArray:
        if len(eachLine)>1:  #Not necessary
            ...
            xar.append(int(x1)) #Including them into an array
            yar.append(int(y1)) 
    ...
    ax1.set_ylim([0, 100]) # max and min value value to Y. Constant throughout.
    ax1.plot(xar,yar)
